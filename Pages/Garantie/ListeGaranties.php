...
// Préparation de la requête SQL avec des jointures optimisées
$sql = "SELECT 
   g.id AS garantie_id, 
   g.num_garantie, 
   g.date_creation, 
   ...
   ao.num_appel_offre,
   dg.id AS document_id, 
   dg.document_path,
   l.id AS liberation_id,
   l.date_liberation
FROM garantie g
...
// Définition de la date actuelle une seule fois
$today = new DateTime();
?>
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Variables globales
            const table = document.getElementById('garantiesTable');
            ...
            function initializeTable() {
                ...
                // Afficher le bouton de suppression de recherche lorsqu'il y a du texte
                searchInput.addEventListener('input', function() {
                    clearSearchBtn.style.display = this.value ? 'block' : 'none';
                });
                
                // Ajouter des écouteurs pour les en-têtes de tri
                document.querySelectorAll('th.sortable').forEach(header => {
                    header.addEventListener('click', function() {
                        ...
                        // Appliquer la classe active et l'ordre
                        this.classList.add('active', order);
                        
                        // Mettre à jour le select de tri
                        sortSelect.value = `${sort}_${order}`;
            ...
            function filterTable() {
                ...
                filteredRows = rows.filter(row => {
                    ...
                    const direction = row.getAttribute('data-direction').toLowerCase();
                    ...
                    // Filtrer par statut
                    const matchesStatus = statusTerm === '' || statut === statusTerm;
            ...
            function sortTable() {
                ...
                filteredRows.sort((a, b) => {
                    ...
                    if (field === 'montant') {
                        valueA = parseFloat(a.getAttribute('data-montant'));
                        valueB = parseFloat(b.getAttribute('data-montant'));
                    } else if (field === 'date_creation' || field === 'date_validite') {
                        valueA = new Date(a.getAttribute(`data-${field === 'date_creation' ? 'date' : 'validite'}`));
                        valueB = new Date(b.getAttribute(`data-${field === 'date_creation' ? 'date' : 'validite'}`));
                    } else if (field === 'num_garantie') {
                        valueA = a.getAttribute('data-num');
                        valueB = b.getAttribute('data-num');
                    } else if (field === 'direction') {
                        valueA = a.getAttribute('data-direction');
                        valueB = b.getAttribute('data-direction');
                    } else if (field === 'fournisseur') {
                        valueA = a.getAttribute('data-fournisseur');
                        valueB = b.getAttribute('data-fournisseur');
                    } else if (field === 'statut') {
                        valueA = a.getAttribute('data-statut');
                        valueB = b.getAttribute('data-statut');
                    }
            ...
            function updateTable() {
                ...
                // Mettre à jour les informations de pagination
                startEntry.textContent = startIndex + 1;
            ...
            function resetFilters() {
                searchInput.value = '';
                statusFilter.value = '';
                sortSelect.value = 'date_creation_desc';
                clearSearchBtn.style.display = 'none';
                
                // Réinitialiser les en-têtes de tri
                document.querySelectorAll('th.sortable').forEach(h => {
                    h.classList.remove('active', 'asc', 'desc');
                });

document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateTable(amendments) {
    ...
    amendments.forEach((amendment) => {
      ...
      row.innerHTML = `
                ...
                <td>
                    ...
                    ${
                      amendment.document_path
                        ? `<a href="../../../PFE_SONATRACH/Backend/Amandements/requetes_ajax/${
                            amendment.document_path
                          }" target="_blank">${
                            amendment.nom_document || "Voir le document"
                          }</a>`
                        : "Aucun document"
                    }
                </td>
                <td>   
                    <div>
                        <button class="modify-btn" data-id="${
                          amendment.id
                        }">Modifier</button>
                    </div>
                </td>
  ...
  tableBody.addEventListener("click", (event) => {
    if (event.target.classList.contains("modify-btn")) {
      ...
      fetch(
        `../../Backend/Amandements/requetes_ajax/get_amendment.php?id=${amendmentId}`
      )
        ...
        .then((data) => {
          if (data.success) {
            ...
            const selectedType =
              document.getElementById("TypeAmandement").value;
            datePronongationField.style.display =
              selectedType == 2 ? "none" : "block";
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!isValidDate(dateAmandement)) {
      ...
      ).textContent = "La date ne peut pas être dans le futur.";
      document.querySelector(
        "#DateAmandement + .validation-message"
      ).style.display = "block";
      isValid = false;
    ...
    if (datePronongationField.style.display !== "none") {
      ...
      const datePronongation =
        document.getElementById("DatePronongation").value;
      if (
        dateAmandement &&
        datePronongation &&
        !isDateAfter(dateAmandement, datePronongation)
      ) {
        document.querySelector(
    ...
    if (
      ...
      !form.querySelector('input[name="amendment_id"]')
    ) {
      ...
      .then((data) => {
        if (data.success) {
          ...
          Swal.fire({
            ...
            title: "Succès!",
            text: isModify
              ? "Amendement modifié avec succès!"
              : "Amendement ajouté avec succès!",
          });
        } else {

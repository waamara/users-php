...
$stmt = $pdo->prepare("
    SELECT users.*, direction.libelle AS structure_libelle, role.nom_role, role.id AS role_id
    FROM users
...
$roles = $pdo->query("SELECT id, nom_role FROM role ORDER BY nom_role")->fetchAll();
?>
            ...
            <div class="info-grid">
                <div>
                    ...
                    <div class="info-group">
                        ...
                        <span class="info-value" id="structureDisplay"><?= htmlspecialchars($user['structure_libelle'] ) ?></span>
                    </div>
                    <div class="info-group">
                        <span class="info-label">Rôle</span>
                        <span class="info-value" id="roleDisplay"><?= htmlspecialchars($user['nom_role']) ?></span>
                    </div>
                </div>
                <div>
                    <div class="info-group">
                    ...
                    <div class="form-group">
                        ...
                        <select id="structure" name="structure" class="form-control">
                            ...
                            <?php foreach ($structures as $s): ?>
                                <option value="<?= $s['id'] ?>" <?= ($user['structure'] == $s['id']) ? 'selected' : '' ?>><?= htmlspecialchars($s['libelle']) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <div class="validation-message"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="role" name="role" class="form-control">
                            ...
                            <?php foreach ($roles as $r): ?>
                                <option value="<?= $r['id'] ?>" <?= ($user['Role'] == $r['id']) ? 'selected' : '' ?>><?= htmlspecialchars(ucfirst($r['nom_role'])) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <div class="validation-message"></div>
                    </div>
...
<script>
    ...
    window.userData = <?= json_encode(array_merge($user, ['role_id' => $user['Role']])) ?>;
</script>

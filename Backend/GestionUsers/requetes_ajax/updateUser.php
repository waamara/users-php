...
$role = intval($input['Role'] ?? 0); // Added role field with capital R to match database column

// Log processed data
error_log("Processed data - ID: $id, Nom: $nomComplet, Username: $userName, Compte: $compte, Structure: $structure, Role: $role");
...
try {
    // Mettre à jour la table users avec le champ Role
    $stmt = $pdo->prepare("UPDATE users SET nom_user = ?, prenom_user = ?, username = ?, status = ?, structure = ?, Role = ? WHERE id = ?");
    $success = $stmt->execute([$nom, $prenom, $userName, $status, $structure, $role, $id]);
